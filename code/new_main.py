...
with open('../data/output.csv', 'w') as fout:
      ...
      for packet in fin:
        ...
        while not success:
            ...
            if halfDecodedPacket:
                ...
                decodedPacket = False
            if encodedPacket == noisePacket: # no interference
                success = True
            elif halfDecodedPacket == halfEncodedPacket: # hamming correction was good
                row["corrections"] += 1
                success = True
            elif halfDecodedPacket == False: # hamming could not correct
                row["retransmissions"] += 1
            else: # bad correction
                if decodedPacket is False: # CRC caught error
                    row["retransmissions"] += 1
                else: # CRC does not catch error
                    row["badReads"] +=1
    csvOut.writerow(row)

    # CRC to hamming with burst noise
    print "CRC->Hamming - Burst...\r"
    row = {}
    row["algorithm"] = "CRC to hamming"
    row["noise"] = "burst"
    row["transmissions"] = 0
    ...
    # CRC to hamming with drift noise
    print "CRC->Hamming - Drfit...\r"
    row = {}
    row["algorithm"] = "CRC to hamming"
    row["noise"] = "drift"
    row["transmissions"] = 0
